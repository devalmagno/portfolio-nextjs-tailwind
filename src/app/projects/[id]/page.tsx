      ...
      <div className="bg-dark-300 rounded-xl flex flex-col overflow-hidden shadow-md mb-12">
        ...
        >
          <Image
            ...
            className={`bg-center bg-cover transition duration-500 ${
              imageExtended ? "object-contain" : "object-cover"
            } sm:object-contain`}
            alt={project.title}
          />
          <>
            <div
              onClick={handleRight}
              className={`hidden sm:block absolute top-[50%] -translate-x-0 translate-y-[-50%] right-5 text-2xl rounded-full p-2 bg-black/20 text-white cursor-pointer ${
                imageExtended ? "group-hover:block" : ""
              }`}
            >
              {" "}
              <FaAngleRight />{" "}
            ...
            <div
              onClick={handleLeft}
              className={`hidden sm:block absolute top-[50%] -translate-x-0 translate-y-[-50%] left-5 text-2xl rounded-full p-2 bg-black/20 text-white cursor-pointer ${
                imageExtended ? "group-hover:block" : ""
              }`}
            >
              <FaAngleLeft />
        ...
        <div className="text-white px-8 py-4 flex flex-col">
          ...
          {project.source === "private project" ? (
            <span className="flex gap-2 items-center text-indigo-600">
              <FaCode /> Code not available | Private Project
            </span>
          ) : (
            <a
              href={project.source}
